CREATE OR REPLACE PROCEDURE `flightboard_contents.p_f3_4_5_sql_execute_agg_chained_tags_search_word_1st`(
  tmp_table STRING, 
  client STRING, 
...
BEGIN
/*==============================================
STORED PROCEDURE::fligtboard_contents.p_f3_4_5_sql_execute_agg_chained_tags_search_word_1st
...
==============================================*/
  ...
  IF suffix = "30d" THEN
    ...
    EXECUTE IMMEDIATE format("""
      DELETE FROM `flightboard_contents.t_chained_tags_search_word_1st_30d`
      WHERE 
        client = "%s"
    ...
    EXECUTE IMMEDIATE format("""
      INSERT INTO `flightboard_contents.t_chained_tags_search_word_1st_30d`(
        client, extract_start_date, extract_end_date, period, month, search_words, count, timestamp
      )
      WITH
      rwl AS (
        SELECT
          ...
          atid,
          gid,
          SPLIT(gid, "_")[offset(2)] AS gid_sort,
          ARRAY_TO_STRING(related_words_log, ",") AS search_words
        FROM `%s` 
      ),
      sw1 AS (
        SELECT * FROM rwl WHERE rwl.gid_sort = "0"
      )
      SELECT
        "%s" AS client,
        ...
        %d AS period,
        SUBSTR(CAST(sw1.dt AS STRING), 1, 7) AS month,
        search_words,
        COUNT(*) AS count,
