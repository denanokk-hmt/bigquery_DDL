CREATE OR REPLACE PROCEDURE `flightboard_contents.p_f3_4_1_sql_execute_agg_cust_uuid`(
  tmp_table STRING, 
  client STRING, 
...
BEGIN
/*==============================================
STORED PROCEDURE::fligtboard_contents.p_f3_4_1_sql_execute_agg_cust_uuid
...
==============================================*/
  ...
  IF suffix = "30d" THEN
    ...
    EXECUTE IMMEDIATE format("""
      DELETE FROM `flightboard_contents.t_cust_uuid_30d`
      WHERE 
        client = "%s"
    ...
    --集計したものをテーブルへ挿入
    EXECUTE IMMEDIATE format("""
      INSERT INTO `flightboard_contents.t_cust_uuid_30d`(
        client, extract_start_date, extract_end_date, period, hmt_id, send_to, talk_cust_type, count, timestamp
      )
      WITH cust_tap AS (
        SELECT
  ...
  ELSEIF suffix = "12m" THEN
    ...
    --集計前に同期間同クライアントの集計結果を削除
    EXECUTE IMMEDIATE format("""
      DELETE FROM `flightboard_contents.t_cust_uuid_12m` AS m
      WHERE 
        1=1
        AND m.client = "%s"
    ...
    EXECUTE IMMEDIATE format("""
      INSERT INTO `flightboard_contents.t_cust_msg_12m`(
        client, extract_month, extract_month_start_date, send_to, talk_cust_type, count, timestamp
      )
      WITH d AS (
