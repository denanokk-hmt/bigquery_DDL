CREATE OR REPLACE PROCEDURE `flightboard_contents.p_f3_4_4_sql_execute_agg_specialtag`(tmp_table STRING, client STRING, extract_start_date DATE, extract_end_date DATE, period INT64, suffix STRING)
BEGIN
/*==============================================
STORED PROCEDURE::fligtboard_contents.p_f3_4_4_sql_execute_agg_specialtag
...
==============================================*/
  ...
  IF suffix = "30d" THEN
    ...
    EXECUTE IMMEDIATE format("""
      DELETE FROM `flightboard_contents.t_specialtag_30d`
      WHERE 
        client = "%s"
    ...
    EXECUTE IMMEDIATE format("""
      INSERT INTO `flightboard_contents.t_specialtag_30d`(
        client, extract_start_date, extract_end_date, period, hmt_id, url, query_tags, customer_uuid, count, timestamp
      )
      SELECT
        "%s" AS client,
