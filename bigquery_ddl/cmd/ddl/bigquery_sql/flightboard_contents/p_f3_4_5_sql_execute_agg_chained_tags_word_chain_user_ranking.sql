CREATE OR REPLACE PROCEDURE `flightboard_contents.p_f3_4_5_sql_execute_agg_chained_tags_word_chain_user_ranking`(
  tmp_table STRING, 
  client STRING, 
...
BEGIN
/*==============================================
STORED PROCEDURE::fligtboard_contents.p_f3_4_5_sql_execute_agg_chained_tags_word_chain_user_ranking

ARGS::
--■■UUワードチェーン利用ユーザーRanking
...
==============================================*/
  ...
  IF suffix = "30d" THEN
      ...
      EXECUTE IMMEDIATE format("""
        DELETE FROM `flightboard_contents.t_chained_tags_word_chain_user_ranking_30d`
        WHERE 
          client = "%s"
      ...
      EXECUTE IMMEDIATE format("""
        INSERT INTO `flightboard_contents.t_chained_tags_word_chain_user_ranking_30d`(
          client, extract_start_date, extract_end_date, period, month, rank, hmt_id, count, timestamp
        )
        WITH
        w AS (
          SELECT 
        ...
        cnt AS (
        SELECT
          SUBSTR(CAST(w.dt AS STRING), 1, 7) AS month,
          hmt_id,
          COUNT(hmt_id) AS count
        FROM w
        GROUP BY 
          hmt_id,
          SUBSTR(CAST(w.dt AS STRING), 1, 7)
        )
        SELECT
          "%s" AS client,
