CREATE OR REPLACE PROCEDURE `flightboard_contents.p_f3_4_5_sql_execute_agg_chained_tags_search_word_ranking`(
  tmp_table STRING, 
  client STRING, 
...
BEGIN
/*==============================================
STORED PROCEDURE::fligtboard_contents.p_f3_4_5_sql_execute_agg_chained_tags_search_word_ranking
...
==============================================*/
  ...
  IF suffix = "30d" THEN
    ...
    EXECUTE IMMEDIATE format("""
      DELETE FROM `flightboard_contents.t_chained_tags_search_word_ranking_30d`
      WHERE 
        client = "%s"
    ...
    EXECUTE IMMEDIATE format("""
      INSERT INTO `flightboard_contents.t_chained_tags_search_word_ranking_30d`(
        client, extract_start_date, extract_end_date, period, month, ranking, word, count, timestamp
      )
      WITH
      rwl AS (
        SELECT
          ...
          word
        FROM `%s`
        ,UNNEST(related_words_log) as word
      ...
      SELECT
        ...
        %d AS period,
        SUBSTR(CAST("%t" AS STRING), 1, 7) AS month,
        ranking,
        word,
        cnt AS count,
      ...
      FROM
        rank
      ;
    """, tmp_table, client, extract_start_date, extract_end_date, period, extract_start_date);
