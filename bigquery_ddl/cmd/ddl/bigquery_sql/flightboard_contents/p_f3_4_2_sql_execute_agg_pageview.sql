CREATE OR REPLACE PROCEDURE `flightboard_contents.p_f3_4_2_sql_execute_agg_pageview`(tmp_table STRING, client STRING, extract_start_date DATE, extract_end_date DATE, period INT64, suffix STRING)
BEGIN
/*==============================================
STORED PROCEDURE::fligtboard_contents.p_f3_4_2_sql_execute_agg_pageview
...
==============================================*/
  ...
  IF suffix = "30d" THEN
    ...
    EXECUTE IMMEDIATE format("""
      DELETE FROM `flightboard_contents.t_pageview_30d`
      WHERE 
        client = "%s"
    ...
    EXECUTE IMMEDIATE format("""
      INSERT INTO `flightboard_contents.t_pageview_30d`(
        client, extract_start_date, extract_end_date, period, hmt_id, url, count, timestamp
      )
      SELECT
        "%s" AS client,
  ...
  ELSEIF suffix = "12m" THEN
    ...
    EXECUTE IMMEDIATE format("""
      DELETE FROM `flightboard_contents.t_pageview_12m` AS m
      WHERE 
        1=1
        AND m.client = "%s"
    ...
    --Mothly集計テーブルへ挿入
    EXECUTE IMMEDIATE format("""
      INSERT INTO `flightboard_contents.t_pageview_12m`(
        client, extract_month, extract_month_start_date, hmt_id, url, count, timestamp
      
      WITH d AS (
        SELECT * FROM `flightboard_contents.t_pageview_30d` 
        WHERE 1=1
          AND client ="%s"
