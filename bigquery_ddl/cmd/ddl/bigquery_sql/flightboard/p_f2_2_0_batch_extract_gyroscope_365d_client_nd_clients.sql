CREATE OR REPLACE PROCEDURE `flightboard.p_f2_2_0_batch_extract_gyroscope_365d_client_nd_clients`(
  job_id STRING, 
  job_name STRING, 
...
BEGIN
/*==============================================
STORED PROCEDURE::flightboard.p_f2_2_0_batch_extract_gyroscope_365d_client_nd_clients
...
==============================================*/
  ...
  FOR ids IN (SELECT client FROM `gcs_logging.tf_get_gyroscope_clients`(CAST(extract_start_date AS DATE), ddiff))
  DO
    --SELECT client, extract_date;
    CALL `flightboard.p_f2_2_3_extract_gyroscope_365d_client_nd`(ids.client, extract_start_date, extract_end_date, NULL, FALSE);

    --ログ格納先テーブル名を設定
    SET destiny_dataset_table = CONCAT("flightboard.t_gyroscope_365d_" , ids.client);

    --ログ格納結果を登録
    CALL `flightboard.p_f2_3_0_write_result_logging_extract`(job_id, job_name, "gyroscope", ids.client, 'daily', extract_start_date, extract_end_date, destiny_dataset_table);
  END FOR;
